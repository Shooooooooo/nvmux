...
JSON and moves nothing, so the socket path — the session's real identity — is
stable and no SSH forward has to be rebuilt.

The metadata lives beside the socket on the session host, not on your laptop, so
attaching from a second machine shows the same names. `$XDG_RUNTIME_DIR` is
deliberately ignored and socket paths are kept short on purpose; `src/config.rs`
explains both.
...
Each session's own output goes to `<id>.log` on the session host. Headless
Neovim writes `:echomsg` *and* errors to stderr, which is why that file is the
first thing to look at when a session will not start.
...
scratch directories. The SSH tests need a host from `$NVMUX_TEST_SSH_HOST`,
defaulting to `selftest`; where none answers they skip rather than fail. Pointing
that alias at localhost exercises the real ssh client, a real ControlMaster, real
unix-socket forwarding and a real remote login shell — everything except latency:
...
There is also no preview pane, and there will not be one: Neovim sizes the
global grid to the minimum across every attached UI, so a small preview would
shrink the grid of the session you are actually editing in. `src/ui/mod.rs` has
the detail.
